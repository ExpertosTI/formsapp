  ...
  return (
    <div className="flex min-h-screen min-h-[100dvh] bg-[var(--tl-bg)]">
      ...
      <div className="flex flex-col flex-1 min-w-0 w-full">
        <header className="sticky top-0 z-30 flex items-center gap-3 px-4 py-3 border-b border-white/[0.06] bg-[var(--tl-surface)]/90 backdrop-blur-xl sm:px-6 pt-[max(0.75rem,env(safe-area-inset-top))]">
          <button
            type="button"
            onClick={toggleMenu}
            ...
            aria-label={desktopOpen ? "Colapsar menú lateral" : "Expandir menú lateral"}
            title={desktopOpen ? "Colapsar menú lateral" : "Expandir menú lateral"}
          >
            {hydrated && desktopOpen ? (
              <PanelLeftClose className="w-5 h-5 text-teal-400" />
            ) : (
